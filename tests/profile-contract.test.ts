...
import type { TestMode, TransferMode } from "../src/types/diagnostics";
...
interface ContractProfile {
  ...
  aggregateDownloadConnections: number;
  aggregateUploadConnections: number;
  includeServices: boolean;
  comparison: {
    singleDownloadDurationMs: number;
    singleDownloadCapBytes: number;
    singleUploadDurationMs: number;
    singleUploadCapBytes: number;
  };
  downloadScaling: Array<{ connections: number; durationMs: number; capBytes: number }>;
}

interface ProfileContract {
  schemaVersion: string;
  transferMethods: TransferMode[];
