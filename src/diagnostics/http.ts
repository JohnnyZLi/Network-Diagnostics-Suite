...
export async function probeR2DownloadPath(signal: AbortSignal, origin = R2_DOWNLOAD_ORIGIN): Promise<R2DownloadProbe> {
  const timed = createTimedSignal(signal, 4_000);
  try {
    const response = await fetch(`${origin}${R2_DOWNLOAD_OBJECT_PATH}`, {
      method: "HEAD",
      credentials: "omit",
      signal: timed.signal
    });
...
export async function downloadR2Range(
  ...
  onObservation: (observation: DownloadDeliveryObservation) => void,
  onRejection: (rejection: DownloadStreamRejection) => void,
  origin = R2_DOWNLOAD_ORIGIN
): Promise<boolean> {
  const range = r2Range(workerIndex, generation);
  let response: Response;
  try {
    response = await fetch(`${origin}${R2_DOWNLOAD_OBJECT_PATH}`, {
      credentials: "omit",
