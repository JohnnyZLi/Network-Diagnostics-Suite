...
export function AdvancedDiagnostics({ profile, method }: { profile: DiagnosticProfile; method: TransferMethod }) {
  const preflightRef = useRef<HTMLElement>(null);
  const [tool, setTool] = useState<AdvancedTool>('targeting');
  ...
  useEffect(() => {
    ...
    const removeStopped = desktopBridge.on<{ port: number }>('lan.server.stopped', () => setServerRunning(false));
    ...
    <section id="advanced-diagnostics" className="workbench-section advanced-workbench" aria-labelledby="advanced-workbench-title">
      <div className="workbench-section-header advanced-workbench-header">
        <div>
          ...
          <h2 id="advanced-workbench-title">Native targeting and LAN tools</h2>
          <p>Specialized controls stay visible in the workbench without turning them into another navigation destination.</p>
        </div>
        <button type="button" className="advanced-primary workbench-save" disabled={busy || loading} onClick={() => void save()}>{busy ? 'Working…' : 'Save configuration'}</button>
      </div>

      <div className="advanced-workbench-tabs" aria-label="Advanced diagnostic tools">
        <button type="button" className={tool === 'targeting' ? 'active' : ''} onClick={() => setTool('targeting')}>
          <strong>Targeting</strong><span>Endpoints · interface · privacy</span>
        </button>
        <button type="button" className={tool === 'lan' ? 'active' : ''} onClick={() => setTool('lan')}>
          <strong>LAN</strong><span>Peer throughput · native server</span>
        </button>
        <button type="button" className={tool === 'preflight' ? 'active' : ''} onClick={() => setTool('preflight')}>
          <strong>Preflight</strong><span>{profileLabel(profile)} · {methodLabel(method)}</span>
        </button>
      </div>
