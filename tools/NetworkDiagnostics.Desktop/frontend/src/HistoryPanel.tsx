...
export function HistoryPanel({
  ...
  const wide = view.kind !== 'list' && view.kind !== 'compare-select';
  ...
  async function openDetail(report: SavedReportSummary) {
    ...
    try {
      ...
      setViewError(value instanceof Error ? value.message : 'The saved report could not be opened.');
    } finally {
      setViewLoading(false);
    }
  }
  ...
  async function compareWith(report: SavedReportSummary, baseline: SavedReportDetail) {
    ...
    try {
      ...
      setViewError(value instanceof Error ? value.message : 'The saved reports could not be compared.');
    } finally {
      setViewLoading(false);
    }
  }
  ...
  function goBack() {
    setViewError(null);
    setView((current) => {
      if (current.kind === 'comparison') return { kind: 'detail', detail: current.baseline };
      if (current.kind === 'compare-select') return { kind: 'detail', detail: current.baseline };
      return { kind: 'list' };
    });
  }
      ...
      >
        <header className="history-header">
          ...
          <div className="history-header-actions">
            {view.kind === 'list' ? (
              ...
              <button type="button" className="history-icon-button" onClick={goBack} aria-label="Back to saved runs">
                <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        {viewLoading ? (
          <div className="history-empty report-loading">
            ...
            <strong>Reading local report data</strong>
            <p>The report remains on this computer while the native engine prepares the view.</p>
          </div>
        ) : view.kind === 'list' ? (
          <ReportList
            reports={reports}
            ...
            error={error}
            onRefresh={onRefresh}
            onOpen={(report) => void openDetail(report)}
          />
          ...
          <CompareSelector
            ...
            error={viewError}
            onCompare={(report) => void compareWith(report, view.baseline)}
          />
        ) : view.kind === 'detail' ? (
          <ReportDetail
            detail={view.detail}
    ...
    <div className="history-body">
      ...
      {reports.length > 0 && (
        <div className="history-list" aria-live="polite">
          {reports.map((report) => (
            <ReportRow key={report.id} report={report} onClick={() => onOpen(report)} />
          ))}
        </div>
      )}
    </div>
    ...
    <div className="report-body">
      ...
      <section className="report-section">
        <div className="report-section-heading">
          <h3>Headline measurements</h3>
          <span>{detail.methodLabel ?? methodLabel(detail.method)}</span>
        </div>
      ...
      <section className="report-section">
        <div className="report-section-heading">
          <h3>Findings</h3>
          <span>{presentation.findings.length}</span>
        </div>
...
function ComparisonDetail({
  ...
  comparison: SavedReportComparison;
  error: string | null;
}) {
  return (
    <div className="report-body">
      ...
      <section className="report-section">
        <div className="report-section-heading">
          <h3>Measurement deltas</h3>
          <span>{comparison.metrics.length}</span>
        </div>
...
function ComparisonReportCard({
  ...
  return (
    <div className="comparison-report-card">
      <span>{label}</span>
      <strong>{displayName(report)}</strong>
...
function headerFor(view: PanelView): { kicker: string; title: string; subtitle: string } {
  if (view.kind === 'detail') {
    return {
      kicker: 'Saved report',
      title: displayName(view.detail.report),
      subtitle: `${formatDateTime(view.detail.report.generatedAt)} · ${view.detail.context || view.detail.report.profileName}`,
    };
  }
  if (view.kind === 'compare-select') {
    return {
      kicker: 'Compare reports',
      title: 'Choose another run',
      subtitle: 'The selected report stays as the baseline.',
    };
  }
  if (view.kind === 'comparison') {
    return {
      kicker: 'Comparison',
      title: `${view.comparison.baseline.profileName} vs ${view.comparison.candidate.profileName}`,
      subtitle: 'Baseline and candidate values are kept side by side.',
    };
  }
  return {
    kicker: 'Local reports',
    title: 'Saved runs',
    subtitle: 'Completed diagnostics stored on this computer.',
  };
}
