...
public static class MonitoringExportService
{
    public static string BuildShareHtml(NetworkExperiencePresentation presentation)
    {
        ...
        builder.Append(".bar{flex:1;min-width:2px;border-radius:2px;background:#55d47e}.bar.laggy{background:#f19758}.bar.down{background:#f06e64}.bar.inactive{background:#392d42}");
        builder.Append(".alerts{margin-top:22px}.alert{border-top:1px solid #382943;padding:12px 0}.muted{color:#8e8397}@media(max-width:720px){.hero{grid-template-columns:1fr}.metrics{grid-template-columns:1fr 1fr}}");
        ...
        {
            var className = sample.State switch
            {
            ...
            builder.Append("<span class=\"").Append(className).Append("\" style=\"height:")
                ...
                .Append(Html(sample.State == MonitorSampleState.Unresponsive ? "Unreachable" : $"{sample.LatencyMs:0.#} ms"))
                .Append("\"></span>");
        }
        ...
        builder.Append("<p class=\"muted\" style=\"margin-top:24px\">Generated locally by Network Diagnostics. Scores summarize the selected time window and are not an Internet service guarantee.</p>");
        ...
        bool includeIdentifiers)
    {
        ...
        var builder = new StringBuilder();
        builder.AppendLine("timestamp_utc,state,latency_ms,jitter_ms,dns_ms,time_to_first_byte_ms,packet_loss_percent,download_mbps,upload_mbps,is_speed_measurement,interface,network_signature");
