...
public static class DiagnosticClassifier
{
    ...
    public static IReadOnlyList<DiagnosticFinding> Classify(NetworkDiagnosticsReportV2 report)
    {
        ...
        if (!findings.Any(item => item.Severity is "warning" or "critical"))
        {
            findings.Insert(0, new DiagnosticFinding(
                ...
                "summary",
                "info",
                report.Run.Profile == TestProfileId.Quick ? "medium" : "high",
                ...
                report.Run.Profile == TestProfileId.Quick ? "Run Full only when you need deeper confirmation." : null));
        }
        ...
        NativeInternetTransferReport internet)
    {
        ...
        {
            var severity = internet.IdleLatency.LossPercent >= rules.ApplicationLatency.RequestLossCriticalPercent
                ? "critical"
                : "warning";
            findings.Add(new DiagnosticFinding(
                ...
                "reliability",
                severity,
                internet.IdleLatency.Sent >= 20 ? "high" : "medium",
                "Requests were lost while the connection was idle",
                "One or more first-party HTTP latency requests failed or timed out. This is application request loss; the native ICMP result is evaluated separately.",
                [
        ...
        else if ((internet.IdleLatency.MedianMs ?? 0) >= rules.ApplicationLatency.IdleMedianWarningMs)
        {
            findings.Add(new DiagnosticFinding(
                ...
                [Evidence("internetTransfer.idleLatency.medianMs", "Idle median", Milliseconds(internet.IdleLatency.MedianMs))],
                ["Compare the gateway and public-target latency below to determine whether the delay starts locally or farther upstream."],
                "Repeat the same profile on Ethernet if possible."));
        }
        ...
        if (single is not null && aggregate is not null && aggregate.SteadyMbps >= rules.Throughput.MinimumAggregateMbpsForFlowComparison)
        {
            ...
            if (share < rules.Throughput.SingleFlowShareWarningPercent)
            {
                findings.Add(new DiagnosticFinding(
                    ...
                    ["Compare another endpoint or time of day before attributing this to the local network."],
                    nextTest));
            }
        }
        ...
        if (Variable(internet.Download) || Variable(internet.Upload))
        {
            findings.Add(new DiagnosticFinding(
                ...
                ["Repeat the same profile after background traffic settles and compare multiple saved reports."],
                nextTest));
        }
        ...
        if (internet.Download.CapReached || internet.Upload.CapReached)
        {
            ...
            findings.Add(new DiagnosticFinding(
                ...
                $"The {directions} phase reached its byte cap before time expired. The result is still useful, but it may be below peak capacity.",
                [Evidence("internetTransfer.dataUsedBytes", "Transferred", Bytes(internet.DataUsedBytes))],
                ["Use Full when you need a more representative throughput measurement."],
                report.Run.Profile == TestProfileId.Quick ? "Run the Full diagnostic." : nextTest));
        }
    }
        ...
        NativeInternetTransferReport? internet)
    {
        ...
        if (deep.LocalLink is not null && internet is not null)
        {
            ...
            if (internetMaximum >= rules.ThroughputFloor() && share < rules.LocalLinkShareWarningPercent)
            {
                findings.Add(new DiagnosticFinding(
                    "local-link-limited",
                    "local-link",
    ...
    {
        TestProfileId.Quick => "Run the Full diagnostic to confirm this finding with longer samples.",
    ...
    private static double ThroughputFloor(this LocalDiagnosticRules _) =>
        Rules.Value.Throughput.MinimumAggregateMbpsForFlowComparison;
}
